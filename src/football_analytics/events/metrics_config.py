...
class MetricsConfigError(EventsContractError):
    """Config failure."""
...
def default_metrics_config_path(*, project_root: Path | None = None) -> Path:
    ...
    return root / "configs" / "events" / "target_metrics_baseline.yaml"
...
def load_metrics_config(
    ...
    if not data.get("requested_metrics"):
        raise MetricsConfigError("requested_metrics required")
